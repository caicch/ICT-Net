...
class CNN_Encoder(nn.Module):
    ...
    def __init__(self, NetType, method, encoded_image_size=14):
        ...
        self.adaptive_pool2 = nn.AdaptiveAvgPool2d((encoded_image_size, encoded_image_size))
        self.adaptive_pool1 = nn.AdaptiveAvgPool2d((encoded_image_size*2, encoded_image_size*2))
    ...
    def forward(self, images):
        ...
        """
        self.NetType = 'resnet101'#vit_b_16
        if 'resnet' in self.NetType:
            #out = self.net(images) #self.net[:6](images),self.net(images)  # (batch_size, 2048, image_size/32, image_size/32)
            #out = self.adaptive_pool(out)  # [batch_size, 2048/512, 8, 8] -> [batch_size, 2048/512, 14, 14]
            # out = out.permute(0, 2, 3, 1)
            out1 = self.net[:6](images)
            out2 = self.net[6](out1)
            out_large = self.adaptive_pool1(out1)
            out_small = self.adaptive_pool2(out2)
