...
def visual_feat_enc(args, encoder_out, attn_weight_s11, attn_weight_s22, attn_weight_l11, attn_weight_l22):
    img_A = imread(args.img_A)
    img_B = imread(args.img_B)
    #dif_feat_small = numpy.absolute((output_s2_list[0][:, 0].view(14,14,512) - output_s1_list[0][:, 0].view(14,14,512)).mean(dim=-1).cpu().numpy())
    # dif_feat_large = numpy.absolute(
    #     (output_l1_list[0][:, 0].view(14, 14, 512) - output_l2_list[0][:, 0].view(14, 14, 512)).mean(
    #         dim=-1).cpu().numpy())
    img_A_feat = attn_weight_l11[0].view(28, 28).cpu().numpy()
    img_B_feat = attn_weight_l22[0].view(28, 28).cpu().numpy()
    for i in range(1):
        # print(i)
        headmap_A = img_A_feat
        headmap_B = img_B_feat

        #headmap_A = dif_feat_small
        #headmap_B = dif_feat_small
        ...
        headmap_A = np.uint8(255 * headmap_A)
        headmap_A = cv2.applyColorMap(255-headmap_A, cv2.COLORMAP_JET)
        superimposed_img_A = headmap_A * 0.3 + img_A * 0.8
        cv2.imwrite('/home/RSC_cap/Example/A/'+str(i) + '.png', superimposed_img_A)

        headmap_B = cv2.resize(headmap_B, (img_A.shape[1], img_A.shape[0]))
        headmap_B = np.uint8(255 * headmap_B)
        headmap_B = cv2.applyColorMap(255-headmap_B, cv2.COLORMAP_JET)
        superimposed_img_B = headmap_B * 0.3 + img_B * 0.8
        cv2.imwrite('/home/RSC_cap/Example/B/' + str(i) + '.png', superimposed_img_B)


def visual_feat(args, encoder_out):
    img_A = imread(args.img_A)
    img_B = imread(args.img_B)
    dif_feat = numpy.absolute((encoder_out[0][:, 0, 512:].view(14,14,512) - encoder_out[0][:, 0, :512].view(14,14,512)).mean(dim=-1).cpu().numpy())
    img_A_feat = encoder_out[0][:, 0, :512].view(14,14,512).mean(dim=-1).cpu().numpy()
    img_B_feat = encoder_out[0][:, 0, 512:].view(14,14,512).mean(dim=-1).cpu().numpy()
    for i in range(1):
        # print(i)
        headmap_A = img_A_feat[:,:]
        headmap_B = img_B_feat[:, :]
...
def visual_feat_dec(args, att_weight_s, att_weight_l, n):
    img_A = imread(args.img_B)
    img_B = imread(args.img_B)
    #dif_feat = numpy.absolute((encoder_out[0][:, 0, 512:].view(14,14,512) - encoder_out[0][:, 0, :512].view(14,14,512)).mean(dim=-1).cpu().numpy())
    img_A_feat = att_weight_s[0, n].view(14, 14).cpu().numpy()
    img_B_feat = att_weight_l[0, n].view(14, 14).cpu().numpy()
    for i in range(1):
        # print(i)
        headmap_A = img_A_feat[:,:]
        headmap_B = img_B_feat[:, :]
        ...
        headmap_A = np.uint8(255 * headmap_A)
        headmap_A = cv2.applyColorMap(255-headmap_A, cv2.COLORMAP_JET)
        superimposed_img_A = (headmap_A * 0.3) + (img_A * 0.7)
        #superimposed_img_A = cv2.addWeighted(headmap_A, 0.5, img_A, 0.5, 0)
        cv2.imwrite('/home/RSC_cap/Example/stage3/'+ str(n) +'_'+'s'+ str(i) + '.png', superimposed_img_A)

        headmap_B = cv2.resize(headmap_B, (img_A.shape[1], img_A.shape[0]))
        headmap_B = np.uint8(255 * headmap_B)
        headmap_B = cv2.applyColorMap(255-headmap_B, cv2.COLORMAP_JET)
        superimposed_img_B = headmap_B * 0.3 + img_B * 0.7
        cv2.imwrite('/home/RSC_cap/Example/stage4/' +str(n) +'_' +'l' + str(i) + '.png', superimposed_img_B)
...
def evaluate_transformer(args,encoder_image,encoder_feat,decoder):
    ...
    word_map_file = '/data/VGRS/RS_change/data-001/WORDMAP_LEVIR_CC_5_cap_per_img_5_min_word_freq.json'#os.path.join(args.data_folder, 'WORDMAP_' + args.data_name + '.json')
    ...
    with torch.no_grad():
        ...
        #encoder_out, output_s1_list, output_s2_list, output_l1_list, output_l2_list = encoder_feat(imgs_A, imgs_B) # encoder_out: (S, batch, feature_dim)
        encoder_out = encoder_feat(imgs_A, imgs_B)
        #print(encoder_feat)

        # 可视化
        #visual_feat_enc(args, encoder_out, output_s1_list, output_s2_list, output_l1_list, output_l2_list)
        ...
        encoder_out = encoder_out.expand(2, S,k, encoder_dim)  # [S,k, encoder_dim]
        encoder_out = encoder_out.permute(0, 2, 1, 3)
        ...
        while True:
            ...
            encoder_out = encoder_out.permute(0, 2, 1, 3)
            pred, att_weight_s, att_weight_l = decoder.transformer(tgt_embedding, encoder_out, tgt_mask=mask)  # (length, batch, feature_dim)
            encoder_out = encoder_out.permute(0, 2, 1, 3)
            visual_feat_dec(args, att_weight_s, att_weight_l, step)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_A',  default='/data/VGRS/RS_change/Levir-CC-dataset-002/images/test/A/test.png')
    parser.add_argument('--img_B',  default='/data/VGRS/RS_change/Levir-CC-dataset-002/images/test/B/test.png')
    parser.add_argument('--data_folder', default="/mnt/data/VGRS/RS_change/data-001/",help='folder with data files saved by create_input_files.py.')
    ...
    parser.add_argument('--path', default="./current_best/saved_model/", help='model checkpoint.') #  ./models_checkpoint/data/2-times/RSICCformer_D/Simis_baseline/
    ...
    for i in range(len(filename)):
        ...
        checkpoint_path = os.path.join(args.path, filename[i])#os.path.join(args.path, "checkpoint_resnet101_MCCFormers_diff_as_Q_trans.pth.tar") #os.path.join(args.path, filename[i])
